...
# ═══════════════════════════════════════════════════════════════════════
# §6.1 / §И.4.1 / §И.4.2 — defensive guards on degenerate inputs
# ═══════════════════════════════════════════════════════════════════════


def test_calc_th_prefill_cb_compute_returns_zero_when_denominator_non_positive() -> None:
    # FPS=0 and eta_pf=0 → denominator = 0 + 4*L*H*sl_pf_eff = positive,
    # so trip the other branch with explicit non-positive Fcount.
    assert (
        calc_th_prefill_cb_compute(
...
def test_calc_sl_pf_vlm_returns_zero_for_negative_visual_tokens() -> None:
    assert calc_sl_pf_vlm(v_tok=-1, n_prompt_txt=10) == 0


def test_calc_sl_pf_vlm_returns_zero_for_negative_prompt() -> None:
    assert calc_sl_pf_vlm(v_tok=10, n_prompt_txt=-1) == 0


def test_calc_l_text_returns_zero_for_zero_inputs() -> None:
    assert calc_l_text(chars_page=0, c_token=3.5) == 0.0
    assert calc_l_text(chars_page=1000, c_token=0) == 0.0


def test_calc_sl_pf_llm_after_ocr_returns_zero_for_negative_inputs() -> None:
    assert calc_sl_pf_llm_after_ocr(l_text=-1.0, n_prompt_sys=100.0) == 0.0
    assert calc_sl_pf_llm_after_ocr(l_text=100.0, n_prompt_sys=-1.0) == 0.0


def test_select_th_prefill_returns_none_when_both_zero() -> None:
    value, mode = select_th_prefill(th_compute=0.0, th_mem=0.0)
    assert value == 0.0
    assert mode == "none"


def test_select_th_prefill_compute_only_when_mem_zero() -> None:
    value, mode = select_th_prefill(th_compute=42.0, th_mem=0.0)
    assert value == 42.0
    assert mode == "compute_only"


def test_select_th_prefill_memory_only_when_compute_zero() -> None:
    value, mode = select_th_prefill(th_compute=0.0, th_mem=99.0)
    assert value == 99.0
    assert mode == "memory_only"


def test_select_th_prefill_picks_min_memory_when_binding() -> None:
    value, mode = select_th_prefill(th_compute=100.0, th_mem=50.0)
    assert value == 50.0
    assert mode == "memory"


def test_select_th_prefill_picks_compute_when_binding() -> None:
    value, mode = select_th_prefill(th_compute=20.0, th_mem=80.0)
    assert value == 20.0
    assert mode == "compute"
...
class TestThDecodeMem:
    ...
    def test_higher_bw_gives_higher_throughput(self) -> None:
        low = calc_th_decode_mem(
            bw_gpu_gbs=1555, eta_mem=0.36, params_billions=7, b_quant=2, mkv_gb=1.83
        )
        high = calc_th_decode_mem(
            bw_gpu_gbs=4800, eta_mem=0.36, params_billions=7, b_quant=2, mkv_gb=1.83
        )
        assert high > low
...
class TestTtftWithOverhead:
    ...
    def test_overhead_adds_to_ttft(self) -> None:
        ...
        with_overhead = calc_ttft(sl_pf_eff=1000.0, th_pf=2000.0, th_dec=1000.0, t_overhead=0.026)
    ...
    def test_default_overhead_is_zero_for_backwards_compat(self) -> None:
        ...
        assert (
            calc_ttft(sl_pf_eff=100.0, th_pf=1000.0, th_dec=500.0) == 100.0 / 1000.0 + 1.0 / 500.0
        )
...
class TestCalcPEffective:
    ...
    def test_pure_moe_no_dense_layer(self) -> None:
        ...
        assert (
            calc_p_effective(p_dense=0, p_moe=100, n_experts=8, k_experts=1, bs_real=1) == 100 / 8
        )
...
class TestCmodelV3Form:
    ...
    def test_bs_one_equals_v2_form(self) -> None:
        ...
        result = calc_Cmodel(
            sl_pf_eff=2000, th_pf=8563.06, Tdec=400, th_dec_per_session=6402.66, bs_real=1
        )
    ...
    def test_bs_real_scales_throughput(self) -> None:
        ...
        result_bs1 = calc_Cmodel(
            sl_pf_eff=2000, th_pf=8563.06, Tdec=400, th_dec_per_session=6402.66, bs_real=1
        )
        result_bs57 = calc_Cmodel(
            sl_pf_eff=2000, th_pf=8563.06, Tdec=400, th_dec_per_session=112.33, bs_real=57
        )
...
class TestCalcKVMLA:
    ...
    def test_deepseek_v3_at_4k_context(self) -> None:
        ...
        result = calc_kv_mla(
            L=61, SL=4000, kv_lora_rank=512, qk_rope_head_dim=64, bytes_state=2, emp_kv=1.0
        )
    ...
    def test_returns_zero_when_both_dims_zero(self) -> None:
        ...
        assert (
            calc_kv_mla(
                L=61, SL=4000, kv_lora_rank=0, qk_rope_head_dim=0, bytes_state=2, emp_kv=1.0
            )
            == 0.0
        )
    ...
    def test_no_factor_of_two_for_k_and_v(self) -> None:
        ...
        result = calc_kv_mla(
            L=1, SL=1, kv_lora_rank=576, qk_rope_head_dim=0, bytes_state=1, emp_kv=1.0
        )
    ...
    def test_emp_kv_scales_linearly(self) -> None:
        ...
        base = calc_kv_mla(
            L=61, SL=1000, kv_lora_rank=512, qk_rope_head_dim=64, bytes_state=2, emp_kv=1.0
        )
        with_pad = calc_kv_mla(
            L=61, SL=1000, kv_lora_rank=512, qk_rope_head_dim=64, bytes_state=2, emp_kv=1.2
        )
...
class TestPDDisaggregationMath:
    ...
    def test_th_server_dec_basic_formula(self) -> None:
        ...
        assert (
            calc_th_server_dec(NcountTP=2, th_dec_per_session=100.0, bs_real=8, Tdec=400.0) == 4.0
        )
...
class TestBatchModeFunctions:
    ...
    def test_n_gpu_batch_basic_formula(self) -> None:
        ...
        assert (
            calc_n_gpu_batch(d_pages=1000, t_page_at_bs_max=30.0, w_seconds=28800, eta_batch=0.9)
            == 2
        )
    ...
    def test_n_gpu_batch_zero_demand_returns_zero(self) -> None:
        ...
        assert (
            calc_n_gpu_batch(d_pages=0, t_page_at_bs_max=30.0, w_seconds=28800, eta_batch=0.9) == 0
        )
    ...
    def test_n_gpu_batch_inf_t_page_returns_inf(self) -> None:
        ...
        assert (
            calc_n_gpu_batch(
                d_pages=1000, t_page_at_bs_max=float("inf"), w_seconds=28800, eta_batch=0.9
            )
            is math.inf
        )
...
class TestMultiClassFunctions:
    ...
    def test_n_gpu_multiclass_basic(self) -> None:
        ...
        assert (
            calc_n_gpu_multiclass(demand_pool_tps=1100.0, th_pool_eff_tps_per_gpu=500.0, k_sla=1.25)
            == 3
        )
