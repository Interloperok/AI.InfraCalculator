...
class GPUInfo(BaseModel):
    ...
    recommended_gpus_per_server: int = Field(
        default=8, description="Рекомендуемое количество GPU на сервер", ge=1, le=16
    )
    ...
    tflops: Optional[float] = Field(
        None,
        description="Пиковая производительность Half Precision / Tensor Core (TFLOPS) для LLM-инференса",
    )
