...
const VLMResultsDisplay = ({ results, loading, error, inputData }) => {
    ...
    return (
      <div className="flex items-center justify-center h-64">
        <div className="text-center">
          ...
          <p className="text-gray-600">Calculating VLM sizing…</p>
        </div>
      </div>
    );
  }
  ...
  if (!results) {
    return (
      <div className="text-center py-12">
        ...
        <p className="text-gray-400">Submit your VLM configuration to see the sizing</p>
      </div>
    );
  }
  ...
  return (
    <div className="gap-6 flex flex-col flex-1">
      <h2 className="text-lg sm:text-2xl font-semibold text-gray-800">VLM Sizing Results</h2>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4" data-tour="vlm-result-cards">
        ...
        <div className="result-tile bg-gradient-to-br from-blue-500 to-indigo-600 rounded-xl p-4 sm:p-6 text-white shadow-lg flex flex-col sm:min-h-[170px] overflow-hidden">
          ...
          <p className="text-xs sm:text-sm opacity-75 mt-2">
            {results.n_repl_vlm || 0} replicas · {results.gpus_per_server || 0} GPU/server
          </p>
        </div>
        ...
        >
          ...
          <p className="text-xs sm:text-sm opacity-90 mt-2">
            {fmt(results.t_page_vlm, 2)}s actual · {fmt(results.sla_page_target, 2)}s target
          </p>
        </div>
        ...
        <div className="result-tile bg-gradient-to-br from-amber-500 to-orange-600 rounded-xl p-4 sm:p-6 text-white shadow-lg flex flex-col sm:min-h-[170px]">
          ...
          <p className="text-xs sm:text-sm opacity-75 mt-2">
            decode {fmt(results.th_dec_vlm, 0)} tok/s
          </p>
        </div>
      </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 text-center">
          <div className="bg-indigo-50 rounded-md py-2 px-2">
            ...
            <p className="text-[10px] text-indigo-600 mt-0.5">
              sustained {fmt(results.sustained_rpm, 0)} pages/min
            </p>
          </div>
          <div className="bg-emerald-50 rounded-md py-2 px-2">
            ...
            <p className="text-[10px] text-emerald-600 mt-0.5">
              sustained {fmt(results.sustained_tpm, 0)}
            </p>
          </div>
          <div className="bg-sky-50 rounded-md py-2 px-2">
            <p className="text-[10px] uppercase font-semibold text-sky-700 tracking-wider">
              TPM Split
            </p>
            <p className="text-sm font-semibold text-sky-900 mt-1">
              in {fmt(results.peak_tpm_input, 0)}
            </p>
            <p className="text-[11px] text-sky-700">
              out {fmt(results.peak_tpm_output, 0)}
            </p>
          </div>
      ...
      <div className="bg-gray-50 rounded-xl p-5" data-tour="vlm-donut-chart">
      ...
      <div className="bg-white border border-gray-200 rounded-xl p-5">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 text-sm">
          ...
          <Stat label="S_TP at Z" value={results.s_tp_z} />
          <Stat
          ...
          <Stat label="SL_pf (eff)" value={fmt(results.sl_pf_vlm_eff, 0)} />
          <Stat label="SL_dec" value={results.sl_dec_vlm} />
          <Stat
            label="KV per page"
