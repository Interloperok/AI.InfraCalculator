  ...
  const basicInputs = (
    <div className="space-y-6">
      ...
      <div className="bg-amber-50 rounded-lg p-4 border border-amber-200" data-tour="sla-targets">
        ...
        {renderSliderInput('ttft_sla', 'TTFT Target (SLA)', 0, 60, 0.5, formData.ttft_sla ?? 1, 'sec', 'Maximum acceptable Time To First Token (seconds). Set to 0 to skip this check.')}
        {renderSliderInput('e2e_latency_sla', 'e2e Latency Target (SLA)', 0, 300, 1, formData.e2e_latency_sla ?? 2, 'sec', 'Maximum acceptable end-to-end latency (seconds). Set to 0 to skip this check.')}
      </div>
    </div>
  );
